# Vars to use in analysis - 4th mode category (transit), further elimination of bad data
...
act <- "bik|wlk" #eco-friendly: bike, walk, transit
trn <- "trn"
dal <- "dal" # drive alone
...
abbr_mode_totals <- data_elim %>%
  ...
  mutate(
    ...
    abbrmode = case_when(
      str_detect(travmode, act) ~ "act",
      str_detect(travmode, trn) ~ "trn",
...
data_mplus_ready <- data_elim %>%
  ...
  # left_join(mode_totals, by = "pid") %>%  #EM TRYING THIS
...
write_rds(data_mplus_ready, here("analysis/data/derived_data/data_4mode-mplus-ready2.rds"))
